...
![Static library folder structure of the nextpLib package showing the Include folder with header files and the Library folder with Linux and ARM platform static library files](assets/api-01.png)

nextpLib is the root folder;

Include folder contains header files;

Library folder contains static library files including library files for Linux platform and ARM platform (programs cross-compiled using the ARM platform are only applicable for use on iNexBot's T30 Teach Pendant);
...
![Qt Designer right-click context menu showing the "Promote to" submenu used to promote a QLineEdit control to the DigitalLineEdit numeric input box widget](assets/api-02.png)

You can see in the right tree structure that the control's Class property changes to DigitLineEdit:

![Qt Designer object tree highlighting the promoted QLineEdit control with its Class property set to DigitLineEdit in the property panel](assets/api-03.png)

Control effect after program runs. Clicking the control will pop up a numeric keypad:

![Numeric keypad popup shown when the promoted DigitLineEdit input box is clicked at runtime, used for entering numeric values](assets/api-04.png)
...
![Qt Designer right-click context menu showing the "Promote to" submenu used to promote a QLineEdit control to the lineEditWidget numeric and text input box widget](assets/api-05.png)

You can see in the right tree structure that the control's Class property changes to lineEditWidget:

![Qt Designer object tree highlighting the promoted QLineEdit control with its Class property set to lineEditWidget in the property panel](assets/api-06.png)

Control effect after program runs. Clicking the control will pop up a numeric and alphabetic keypad:

![Numeric and alphabetic keypad popup shown when the promoted lineEditWidget input box is clicked at runtime, used for entering mixed numeric and text values](assets/api-07.png)

## Demo Instructions

![NextpMode demo folder structure diagram showing the settingparawidget source files, widgetmanager management class, and the nextplib static library folder required by the demo](assets/api-08.png)
...
![Setting parameter user-defined form rendered from settingparawidget.ui, used as the demo's custom widget for entering parameter values](assets/api-09.png)
...
![Teach pendant login screen showing the Operator role selection with Administrator option and the password entry field for demo access](assets/api-10.png)

Click [Process] button on the left > User to enter the custom form

![Teach pendant main menu with the Process entry highlighted on the left toolbar used to open the custom parameter form](assets/api-11.png)

![Teach pendant custom parameter form launched from the Process menu, displaying the user-defined widget defined in settingparawidget.ui](assets/api-12.png)

Click the modify button to modify parameters. Click save to send parameters to the controller

![Teach pendant custom parameter form showing the Modify button used to switch input fields into edit mode and the Save button used to send the values to the controller](assets/api-13.png)

The QtCreator console will print the data sent to the controller

![QtCreator application output panel printing the JSON message sent to the controller after clicking Save on the custom parameter form](assets/api-14.png)

![JSON message details logged in the QtCreator console, showing the robot number, booldata, data, and name fields assembled for the controller](assets/api-15.png)
...
![Teach pendant Process main interface with all built-in technology tool buttons hidden, leaving only the user-defined custom buttons visible after calling hideTechnologyToolbuttons](assets/api-16.png)
