...
![Example custom instruction interface template from the teach pendant secondary development demo, providing input fields for ID, motion type, velocity, acceleration, deceleration, and smoothness parameters](assets/sj-demo-01.png)
...
![Teach pendant instruction insertion page showing the two registered custom instruction names "Switch Gun Action" and "Move to Switch Gun Position" listed under the custom instruction class](assets/sj-demo-02.png)
...
![Teach pendant job file showing the inserted custom instruction "Move to switch gun position 1 0 50 50 50 0" with the packaged parameter string appearing as a new line](assets/sj-demo-03.png)
...
![Teach pendant custom instruction interface showing the Cancel button bound to slot_cmdInsertCancelClicked, which closes the custom interface and returns to the previous level without inserting an instruction](assets/sj-demo-04.png)
