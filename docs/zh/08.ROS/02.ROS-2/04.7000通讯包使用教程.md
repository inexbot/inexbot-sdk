...
![SSH 登录控制器后修改 controller.json 文件权限](assets/ros2-01.png)

### 关闭 CRC 校验

打开 `controller.json`，在 `hostComputerService` 节点下找到 `checkCRC`，将 `true` 改为 `false`。

![controller.json 文件中找到 hostComputerService 节点的 checkCRC 字段](assets/ros2-02.png)

![将 checkCRC 由 true 改为 false 关闭 CRC 校验](assets/ros2-03.png)
...
![nrc_rostopic_joint 节点启动后显示成功连接控制器的日志](assets/ros2-04.png)

![与控制器握手成功后的数据交互日志](assets/ros2-05.png)
...
![rostopic_joint 演示程序运行提示输入数字 1-6](assets/ros2-06.png)
...
![示教器中新建并打开工程](assets/ros2-07.png)

插入指令，选择：**运动控制类 → 外部点**

![示教器指令树中选择运动控制类的外部点指令](assets/ros2-08.png)

然后将示教器切为运行模式，运行该工程。

![示教器切换为运行模式](assets/ros2-09.png)

通过监控，可以看出此时机器人在直角坐标系下的点位。

![示教器监控面板显示机器人当前直角坐标系下的点位](assets/ros2-10.png)

在运行 `rostopic_joint.cpp` 的终端中输入 5，此时在运行 `nrc_rostopic_joint.cpp` 的终端中会出现以下效果。

![输入 5 后 nrc_rostopic_joint 终端输出发送给控制器的目标点位](assets/ros2-11.png)

机器人开始移动，最终在直角坐标系下的点位变换成我们发给控制器的点位。注意：若没有运行，可以按下 `START` 试试。

![机器人运动到姿态 1 后示教器监控显示新点位](assets/ros2-12.png)

然后在运行 `rostopic_joint.cpp` 的终端中输入 6，点击 `START`，则机器人运行到 case 6 中的点位。

![机器人运动到姿态 2 后示教器监控显示对应点位](assets/ros2-13.png)
