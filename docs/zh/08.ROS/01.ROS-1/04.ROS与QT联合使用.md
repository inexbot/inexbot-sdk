...
![Qt 工程根目录文件列表，包含 main.cpp、mainwindow.cpp、mainwindow.h、mainwindow.ui 和 Qt_test.pro](assets/ros-23.png)

![Qt 工程 libs 目录结构，包含 include 文件夹和 libnrc_host.so 动态链接库](assets/ros-24.png)

![Qt 工程 include 目录下的三个子文件夹：c_interface、cpp_interface 和 parameter](assets/ros-25.png)

Qt 工程需要略微调整结构才能作为 ROS 功能包使用，如下所示：

![调整为 ROS 功能包后的 Qt_test 目录结构，包含 include、libs、src 文件夹以及 CMakeLists.txt 和 package.xml](assets/ros-26.png)

其中，将原工程中的 `main.cpp`、`mainwindow.cpp`、`Qt_test.pro` 文件放到 `src` 中，`mainwindow.h` 放到 `include/Qt_test` 中：

![Qt_test/src 和 Qt_test/include/Qt_test 目录结构调整后的截图](assets/ros-27.png)
...
![catkin_make 编译 Qt_test 功能包成功的终端输出截图](assets/ros-28.png)

![Linux 桌面上生成的 Qt_test 可执行文件图标](assets/ros-29.png)
...
![运行 rosrun Qt_test Qt_test 后启动的 Qt 机器人控制界面，显示 IP 连接、控制器选择以及臂1和臂2的关节控制矩阵](assets/ros-30.png)
