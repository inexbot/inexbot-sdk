...
**函数原型**：
```cpp
Result set_tool_hand_number(SOCKETFD socketFd, int toolNum);
```
...
// 设置工具手编号为3
Result result = set_tool_hand_number(fd, 3);
if (result == 0) {
    printf("设置工具手编号成功\n");
} else {
    printf("设置工具手编号失败\n");
...
**函数原型**：
```cpp
Result get_tool_hand_number(SOCKETFD socketFd, int& toolNum);
```
...
|------|------|------|------|
| socketFd | SOCKETFD | 连接句柄 | - |
| toolNum | int& | 输出参数，当前工具手编号 | [0, 999] |
...
```cpp
int toolNum;
Result result = get_tool_hand_number(fd, toolNum);
if (result == 0) {
    printf("当前工具手编号：%d\n", toolNum);
}
```
...
**函数原型**：
```cpp
Result set_tool_hand_param(SOCKETFD socketFd, int toolNum, ToolParam param);
```
...
struct ToolParam {
    ...
    double b;      // 绕Y轴旋转，单位弧度
    double c;      // 绕Z轴旋转，单位弧度
    double payloadMass;        // 负载质量，单位kg
    double payloadInertia;     // 负载惯量，单位0.001kgm²
    double payloadMassCenterX; // 负载质心X，单位m
    double payloadMassCenterY; // 负载质心Y，单位m
    double payloadMassCenterZ; // 负载质心Z，单位m
};
```
...
param.x = 0.0;
param.y = 0.0;
param.z = 150.0;  // 工具长度150mm
param.a = 0.0;
param.b = 0.0;
param.c = 0.0;
param.payloadMass = 2.0;  // 负载2kg
param.payloadInertia = 0.0;
param.payloadMassCenterX = 0.0;
param.payloadMassCenterY = 0.0;
param.payloadMassCenterZ = 0.075;  // 负载质心位置

Result result = set_tool_hand_param(fd, 1, param);
if (result == 0) {
    printf("设置工具手参数成功\n");
}
```
...
**函数原型**：
```cpp
Result get_tool_hand_param(SOCKETFD socketFd, int toolNum, ToolParam& param);
```
...
| socketFd | SOCKETFD | 连接句柄 | - |
| toolNum | int | 工具手编号 | [1, 999] |
| param | ToolParam& | 输出参数，工具手参数 | - |

**使用示例**：
```cpp
ToolParam param;
Result result = get_tool_hand_param(fd, 1, param);
if (result == 0) {
    printf("工具手参数：\n");
    printf("  X偏移：%.2f mm\n", param.x);
    printf("  Y偏移：%.2f mm\n", param.y);
...
**函数原型**：
```cpp
Result tool_hand_2_or_20_point_calibrate(SOCKETFD socketFd, int pointNum);
```
...
|------|------|------|------|
| socketFd | SOCKETFD | 连接句柄 | - |
| pointNum | int | 标定点编号 | 2点法[1,2]，20点法[1,20] |
...
**函数原型**：
```cpp
Result tool_hand_2_or_20_point_calibrate_caculate(SOCKETFD socketFd, int toolNum, bool is2Point);
```
...
| socketFd | SOCKETFD | 连接句柄 | - |
| toolNum | int | 工具手编号 | [1, 999] |
| is2Point | bool | 是否为2点标定 | true: 2点法，false: 20点法 |

**使用示例**：
```cpp
// 20点标定完成后计算
Result result = tool_hand_2_or_20_point_calibrate_caculate(fd, 1, false);
if (result == 0) {
    printf("20点标定计算成功\n");
}
```
...
**函数原型**：
```cpp
Result tool_hand_2_or_20_point_calibrate_clear(SOCKETFD socketFd, int toolNum, bool is2Point);
```
...
**函数原型**：
```cpp
Result tool_hand_7_point_calibrate(SOCKETFD socketFd, int pointNum, int toolNum);
```
...
for (int i = 1; i <= 7; i++) {
    ...
    // 标记点位
    Result result = tool_hand_7_point_calibrate(fd, i, 1);
    if (result == 0) {
        printf("标定第%d个点成功\n", i);
    } else {
        printf("标定第%d个点失败\n", i);
...
**函数原型**：
```cpp
Result tool_hand_7_point_calibrate_caculate(SOCKETFD socketFd, int toolNum, int calibrationPointNum);
```
...
| socketFd | SOCKETFD | 连接句柄 | - |
| toolNum | int | 工具手编号 | [1, 999] |
| calibrationPointNum | int | 标定点数 | 6 或 7 |

**使用示例**：
```cpp
// 7个点全部标定完成后，计算标定结果
Result result = tool_hand_7_point_calibrate_caculate(fd, 1, 7);
if (result == 0) {
    printf("标定计算成功\n");
} else {
    printf("标定计算失败，请重新标定\n");
...
**函数原型**：
```cpp
Result tool_hand_7_point_calibrate_clear(SOCKETFD socketFd, int toolNum, int calibrationPointNum);
```
