...
| 字段 | 类型 | 说明 |
|------|------|------|
| msgType | int | 消息类型：0=普通消息，1=警告消息，2=错误消息（22.07 版本无此参数，全部设为 0） |
...
| num | int | 远程 IO 数量，默认 10（22.07 版本无此参数；24.03 版本以上可修改） |
...
| num | int& | 输出参数，远程 IO 数量（22.07 版本调用此接口 num 返回 -1） |
...
| socketFd | SOCKETFD | 连接句柄 |
| robotNum | int | 机器人编号 (1-4) |
| num | int& | 输出参数，远程 IO 数量（22.07 版本返回 -1） |
| time | int& | 输出参数，IO 重复触发屏蔽时间 (ms) |
...
| program | std::vector\<RemoteProgram\> | 远程状态提示参数（24.03 版本 size 必须与 set_remote_param 的 num 相等；22.07 版本必须等于 10） |
...
| socketFd | SOCKETFD | 连接句柄 |
| robotNum | int | 机器人编号 (1-4) |
| program | std::vector\<RemoteProgramSetting\> | 远程程序设置，22.07 版本固定大小为 10 |
