...
在 Cpp 文件夹下有 include 头文件夹，windows 文件夹下 win_mingw64_v2.x.x 存放着最新版本的动态库。
...
└── qt_demo.pro
```
在项目中的 qt_demo.pro 文件的最后添加上 SDK 的头文件以及库引用
...
在 Cpp 文件夹下有 include 头文件夹，windows 文件夹下 win_msvc2017_x64_v2.x.x 存放着最新版本的动态库。

#### 2.1 创建项目

选择 c++ “控制台应用”创建。

![在 Visual Studio 的创建新项目向导中选择 C++"控制台应用"模板](assets/cpp-03.png)
...
![Linux 下 C++项目的目录结构，包含 include、lib、src 文件夹和 Makefile 构建文件](assets/cpp-05.png)

### 1、新建 Makefile 文件

将下面内容拷贝到 Makefile 文件中，并保存。
