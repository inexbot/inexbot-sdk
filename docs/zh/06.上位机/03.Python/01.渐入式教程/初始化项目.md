...
根据自己的 python 版本在相关下载相关下载对应 python 版本的库，如果版本不对应，会报找不到 nrc_host.so 文件

将下载的_nrc_host.so 和 nrc_interface.py 放到项目中。

![Python 项目目录下包含 main.py、_nrc_host.so 共享库和 nrc_interface.py 接口模块三个文件](assets/python-01.png)
```py
## -*- coding: UTF-8 -*-
