...
本协议所规定的所有数据包均符合本格式定义，采用标准的 TLV（Type,Length,Value）格式，有固定起始包头和 CRC 校验。
...
| Length | 2 | 16进制长度0-0xffff；不包含数据头和命令字，不包含 CRC 段 |
| Command | 2 | 命令字 |
...
在配置文件 controller.json 中，`hostComputerService`节点下：

- `socketType`：socket 类型，可选择"TCP"和"UDP"，默认为"TCP"
- `checkCRC`：是否进行 CRC 检测，选择 false 时，不检测 CRC，但注意 CRC 所占用的4个字节的数据仍要发送；默认为 true，检测 CRC
...
| interval | int | 否 | ms，仅 mode 为持续时有效，范围10-60000 |
...
| IO | array | 可查询的 IO 名数组，如["DI1", "DI16", "DO1", "DO3", "DO17"]，最大数量不应大于 IO 个数 |
...
| IO | object | IO 端口，键为 IO 名，值为 IO 状态（0或1） |
