...
| variable | object | 变量；int 类型或 bool 类型或 double 类型或 string 类型变量，全局或局部 |
| variableValue | object | 变量值；手填值或 bool 类型或 int 类型或 double 类型变量 |
...
| userParamInt | int | 二次开发中客户自定义，目前无意义 |
| userParamString | string | 二次开发中客户自定义，目前无意义 |
| variable | object | 变量名；int 类型或 bool 类型或 double 类型，全局 |
