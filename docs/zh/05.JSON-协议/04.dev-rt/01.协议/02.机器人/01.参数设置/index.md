...
| externJointParam | array | 外部轴关节参数列表；已采集帧中包含1项 |
...
| externJointParam[].encoderResolution | int | 编码器分辨率 |
| externJointParam[].encoderResolutionUnit | int | 编码器分辨率单位，范围[0,1] |
...
| externJointParam[].angleToDistanceUnit | int | 角度距离转换比单位，范围[0,1] |
...
- **命令字：** `0x20A8`
> 回复参数与设置参数一致，`externJointParam` 数组元素额外包含 `axisNum`。
