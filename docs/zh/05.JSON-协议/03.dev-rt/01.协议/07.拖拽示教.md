...
| 参数名 | 类型 | 取值范围 | 说明 |
|--------|------|----------|------|
| mode | int | [0,3] | 拖动方式：0-无，1-3D 鼠标，2-力矩，3-位置 |
| port | int | 当前绑定 IO 端口数 | 外部触发信号端口 |
...
| Start | bool | - | 开始/停止记录：true-开始记录，false-停止记录；false 时"MaxSamplingNum"和"SamplingInterval"可以不存在 |
