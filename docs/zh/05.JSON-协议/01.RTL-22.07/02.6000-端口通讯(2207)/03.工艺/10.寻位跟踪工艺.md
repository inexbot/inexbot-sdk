...
![设置激光器参数0x4130 TRACK_LASER_PARAM_SET 的 JSON 数据格式](assets/json-09.png)

设置激光器配置参数

![设置激光器配置参数0x4130 TRACK_LASER_PARAM_SET 的 JSON 数据格式](assets/json-10.png)
...
![激光器标定0x4140 SENSOR_LASER_CALIBRATE_INQUIRE 的 JSON 数据格式](assets/json-11.png)
...
![设置寻位类型0x4133 LOCATING_SENSORTYPE_SET 的 JSON 数据格式](assets/json-12.png)
...
![设置跟踪类型0x4169 TRACK_SENSORTYPE_SET 的 JSON 数据格式](assets/json-13.png)
...
![设置激光跟踪参数表0x4136 TRACK_LASER_TRACKPARAM_SET 的 JSON 数据格式](assets/json-14.png)
...
| robot | int | 是 | 机器人编号 |
| tableid | int | 是 | 跟踪模式 |
| trackParam.compensateX | float | 否 | x 方向补偿量 |
| trackParam.compensateY | float | 否 | y 方向补偿量 |
| trackParam.compensateZ | float | 否 | z 方向补偿量 |
| trackParam.din_end | int | 否 | 跟踪结束输入 IO |
| trackParam.dout_part_move | int | 否 | 工件旋转输出 IO |
...
| trackParam.positionHold.switchon | bool | 否 | 寻位保存功能：true 开启，false 关闭 |
...
![寻位参数表0x4139 TRACK_LASER_SEARCHPARAM_SET 的 JSON 数据格式](assets/json-15.png)
...
| robot | int | 是 | 机器人编号 |
| tableid | int | 是 | 参数表编号 |
| searchParam.compensateX | float | 否 | x 方向补偿量 |
| searchParam.compensateY | float | 否 | y 方向补偿量 |
| searchParam.compensateZ | float | 否 | z 方向补偿量 |
...
![复制寻位参数0x413C TRACK_SEAMTRACK_PARAM_COPY 的 JSON 数据格式](assets/json-16.png)

复制寻位参数（复制前要0x413D 清空）
...
![电弧跟踪工艺0x4150 TRACK_ARC_COMMPARAM_SET 的 JSON 数据格式](assets/json-17.png)

设置通讯参数

![设置通讯参数0x4150 TRACK_ARC_COMMPARAM_SET 的 JSON 数据格式](assets/json-18.png)
...
![设置左右补偿参数0x4153 TRACK_ARC_LATERALCOMPENPARAM_SET 的 JSON 数据格式](assets/json-19.png)
...
![设置高低补偿参数0x4156 TRACK_ARC_VERTICALCOMPENPARAM_SET 的 JSON 数据格式](assets/json-20.png)
...
![触摸寻位参数0x4160 SEARCH_TOUCH_PARAM_SET 的 JSON 数据格式](assets/json-21.png)
...
![弧压追踪参数0x4163 ARC_VOLTAGE_TRACK_PARAMETERS_SET 的 JSON 数据格式](assets/json-22.png)
