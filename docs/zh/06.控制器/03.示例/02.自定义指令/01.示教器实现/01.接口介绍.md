...
![插入自定义指令时打开的用户自定义界面示例](assets/sj-01.png)

cmdNum 参数：在指令插入界面的自定义指令类内从上到下的第几个：

![指令插入界面中自定义指令从上到下编号顺序示意图](assets/sj-02.png)

### signal_userdefine_cmd_alter：用于修改已经插入到作业文件中的自定义指令，调用后同样会打开用户自己写的自定义指令的界面

![修改已插入的自定义指令时打开的用户自定义界面示例](assets/sj-03.png)

cmdNum 参数：需要修改的是哪一条自定义指令，具体哪一条指令按照下图去分辨：

![自定义指令列表中各指令编号的对应关系示意图](assets/sj-02.png)
...
```
![调用 userdefined_cmd_name_change 接口修改插入界面自定义指令名称后的效果示例](assets/sj-04.png)
