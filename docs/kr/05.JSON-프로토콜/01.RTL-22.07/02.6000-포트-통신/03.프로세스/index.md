...
| [[스프레이프로세스](./07.스프레이.md) | 0x4701~0x4718 | 아날로그설정、숫자量설정、궤적매개변수、수동조작 |
...
| [[레이저 절단프로세스](./06.레이저-절단/index.md) | 0x4401~0x4423 | IO포트설정、전역매개변수、아날로그 매칭、절단매개변수、포인트射매개변수、수동조작 |
...
| 0x400E | WELDFACTCURVOL_SET | 설정전류-전압 매칭실제값 |
...
| 0x4707 | SPRAY_SEQUENTIAL_SET | 설정타이밍 |
| 0x4708 | SPRAY_SEQUENTIAL_INQUIRE | 조회타이밍 |
| 0x4709 | SPRAY_SEQUENTIAL_RESPOND | 반환타이밍 |
| 0x470A | SPRAY_TRAJECTORY_SET | 설정궤적매개변수 |
| 0x470B | SPRAY_TRAJECTORY_INQUIRE | 조회궤적매개변수 |
| 0x470C | SPRAY_TRAJECTORY_RESPOND | 반환궤적매개변수 |
| 0x4711 | SPRAY_HAND_OPERATION_SET | 설정수동조작 |
| 0x4712 | SPRAY_HAND_OPERATION_INQUIRE | 조회수동조작 |
| 0x4713 | SPRAY_HAND_OPERATION_RESPOND | 반환수동조작상태 |
...
| 0x4107 | VISION_DEBUGGING_POS_INQUIRE | 조회비전调试포인트열테이블 |
| 0x4108 | VISION_DEBUGGING_POS_RESPOND | 반환调试포인트열테이블 |
| 0x4109 | VISION_DEBUGGING_POS_CLEAR | 비우기调试포인트열테이블 |
| 0x410A | VISION_DEBUGGING_TAKE_PICTURE | 拍照 |
| 0x410B | VISION_DEBUGGING_CALCULATE | 计算偏移 |
| 0x410C | VISION_DEBUGGING_POS_MOVE | 모션至该포인트 |
...
| 0x4807 | TRACK_CONVEYOR_REALTIME_INQUIRE | 实时조회엔코더값和속도 |
...
| 0x4812 | RACK_CONVEYOR_CALIBRATION_INQUIRE | 조회已캘리브레이션的포인트좌표 |
| 0x4813 | TRACK_CONVEYOR_CALIBRATION_RESPOND | 반환캘리브레이션的포인트좌표 |
| 0x4814 | TRACK_CONVEYOR_CALIBRATION_CLEAR | 비우기캘리브레이션값 |
...
| 0x800C | PUNCH_SINGLE_RUN_TEST | 단일 머신试실행 |
...
| 0x8011 | PUNCH_ONLINE_STATUS_RESET | 온라인상태리셋 |
| 0x8012 | PUNCH_JOBFILE_OPERATION | 일시정지/시작/정지 |
| 0x8013 | PUNCH_INQUIRE_RUN_STATUS | 실행주기조회 |
| 0x8014 | PUNCH_RESPOND_RUN_STATUS | 반환실행상태 |
| 0x8015 | PUNCH_SEND_ERROR_INFO | 전송오류팝업정보 |
| 0x8016 | PUNCH_RECEIVE_ERROR_DEAL | 수신팝업按键处理 |
| 0x8017 | PUNCH_CHOOSE_CRAFT_NUM | 选中프로세스번호 |
| 0x8018 | PUNCH_APPLY_TO_ONLINE | 应用到온라인 |
| 0x8019 | PUNCH_INQUIRE_CURRENT_CRAFT_NUM | 조회현재选中프로세스번호 |
| 0x801A | PUNCH_RESPOND_CURRENT_CRAFT_NUM | 반환현재选中프로세스번호 |
...
| 0x4410 | LASER_SHOTPARM_SET | 포인트射매개변수설정 |
| 0x4411 | LASER_HANDOP_SET | 수동조작설정 |
| 0x4412 | LASER_HANDOP_INQUIRE | 수동조작상태조회 |
| 0x4413 | LASER_HANDOP_RESPOND | 수동조작상태응답 |
...
### 프로세스번호 (craftID/num/visionNum/conveyorID等)

| 유형 | 설명 |
|------|------|
| int | 프로세스번호，아님同프로세스유형有아님同的값 범위범위 |
...
1. **메시지ID형식**：所有메시지ID使用16진수형식，如 `0x4001`
