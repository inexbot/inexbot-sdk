...
우리 컨트롤러에는 세 가지 모션이 있습니다: 示教 모드, 運줄 모드, 遠程 모드.

그 중 示教 모드에는 다시 다음이 있습니다: 拖拽 示教, 조그 示教.

이번 회에서는 조그 示教 하에서 단일 포인트 모션을 수행하는 방법을 소개합니다.

## 1. 전원 켜기 함수 캡슐화

단일 포인트 모션은 조그 示教 하에서 수행되지만, 示教 모드의 모션은 추가적인 전원 켜기 작업이 필요하므로 전원 켜기 함수를 캡슐화해야 합니다.
...
int main()
{
    ...
    set_current_mode(fd, 0);   // 示教 모드 설정, 示教 모드의 모션만 전원 켜기 필요
