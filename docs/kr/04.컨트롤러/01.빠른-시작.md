...
> 여기를 클릭 다운로드得기반Windows시스템VirtualBox소프트웨어설치包，如필요其他시스템버전请自줄 VirtualBox공식 사이트 가져오기줄다운로드。
> •
...
새로 생성一번째 demo 파일夹，값을头파일放include파일夹，libNexRob.a를lib파일夹。，디렉터리结构참조다음과 같이：
...
> 등록：如果예x64아키텍처라면컨트롤러필요값을g++-4.8수정이(가)g++-9，-m32수정이(가)-m64，如果컨트롤러예x86아키텍처라면가능不사용하여가져오기줄수정。
