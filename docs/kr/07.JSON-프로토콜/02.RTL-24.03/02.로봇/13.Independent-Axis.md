# 독립 축

## 독립 축매개변수

### 설정독립 축인터페이스매개변수

**명령字：** `0x2800`

**매개변수설명：**

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| axisNum | int | 축编号 |
| angularDistanceConversionRatioValue | double | 转换比 |
| isTrack | bool | 여부예地轨 |
| encoderBitLineditValue | double | 엔코더비트数 |
| inverseLimitValue | double | 反리밋 |
| maximumPositiveSpeedValue | double | 最大正转速；단위:倍率 |
| maximumReverseSpeedValue | double | 最大反转速；단위:倍率 |
| motorDir | int | 모터方向；1：正，-1：负 |
| positiveLimitValue | double | 正리밋 |
| ratedPositiveSpeedValue | double | 额定正转速；단위:rpm |
| ratedReverseSpeedValue | double | 额定反转速；단위:rpm |
| reductionRatioValue | double | 减速比 |
| maxAccValue | double | 最大加속도；단위:속도的倍率 |
| maxDecValue | double | 最大减속도；단위:속도的倍率 |
| selectBindServo | uint | 选择필요바인딩的서보编号 |

**요청예시：**

```json
{
  "independentAxisParameter": {
    "axisNum": 0,
    "angularDistanceConversionRatioValue": 0.0,
    "isTrack": false,
    "encoderBitLineditValue": 0.0,
    "inverseLimitValue": 0.0,
    "maximumPositiveSpeedValue": 0.0,
    "maximumReverseSpeedValue": 0.0,
    "motorDir": 0,
    "positiveLimitValue": 0.0,
    "ratedPositiveSpeedValue": 0.0,
    "ratedReverseSpeedValue": 0.0,
    "reductionRatioValue": 0.0,
    "maxAccValue": 0.0,
    "maxDecValue": 0.0,
    "selectBindServo": 0
  }
}
```

### 호스트 컴퓨터조회독립 축인터페이스매개변수

**명령字：** `0x2801`

**매개변수설명：**

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| axisNum | int | 축编号 |

**요청예시：**

```json
{
  "axisNum": 1
}
```

### 컨트롤러 응답독립 축인터페이스매개변수

**명령字：** `0x2802`

반환매개변수同 `0x2800` 설정명령。

### 호스트 컴퓨터조회독립 축总数

**명령字：** `0x2803`

**매개변수설명：**

없음요청매개변수。

### 컨트롤러 응답독립 축总数

**명령字：** `0x2804`

**매개변수설명：**

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| axisSum | int | 독립 축总数 |

**응답예시：**

```json
{
  "axisSum": 2
}
```

### 호스트 컴퓨터삭제某一번째독립 축매개변수

**명령字：** `0x2805`

**매개변수설명：**

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| axisNum | int | 축编号 |

**요청예시：**

```json
{
  "axisNum": 2
}
```

## 독립 축위치

### 호스트 컴퓨터조회독립 축위치

**명령字：** `0x2806`

**매개변수설명：**

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| axisNum | int | 축编号 |

**요청예시：**

```json
{
  "axisNum": 2
}
```

### 컨트롤러 응답독립 축위치

**명령字：** `0x2807`

**매개변수설명：**

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| currentPos | double | 현재위치 |

**응답예시：**

```json
{
  "currentPos": 210.01
}
```

### 독립 축0点캘리브레이션

**명령字：** `0x2808`

**매개변수설명：**

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| axisNum | int | 축编号 |

**요청예시：**

```json
{
  "axisNum": 2
}
```

## 축独立제어弹窗

### 独立제어축PV모션

**명령字：** `0x30E0`

**매개변수설명：**

| 매개변수名 | 유형 | 설명 | 取值범위 |
|--------|------|------|----------|
| robot | int | 로봇编号 | [1,4] |
| axisNum | int | 축编号 | - |
| vel | int | 축속도；단위:rpm | [-6000,6000] |
| dir | int | 모션方向；1：正，-1：反向 | - |
| acc | double | 加속도 | - |
| dec | double | 减속도 | - |

**요청예시：**

```json
{
  "robot": 1,
  "axisNum": 1,
  "vel": 50,
  "dir": 1,
  "acc": 50.0,
  "dec": 50.0
}
```

### 独立제어축PV정지

**명령字：** `0x30E1`

**매개변수설명：**

| 매개변수名 | 유형 | 설명 | 取值범위 |
|--------|------|------|----------|
| robot | int | 로봇编号 | [1,4] |
| axisNum | int | 축编号 | - |

**요청예시：**

```json
{
  "robot": 1,
  "axisNum": 1
}
```

### 取消축独立제어모션

**명령字：** `0x30E4`

**매개변수설명：**

| 매개변수名 | 유형 | 설명 | 取值범위 |
|--------|------|------|----------|
| robot | int | 로봇编号 | [1,4] |
| axisNum | int | 축编号 | - |

**요청예시：**

```json
{
  "robot": 1,
  "axisNum": 1
}
```

### 독립 축원점 복귀

**명령字：** `0x30E5`

**매개변수설명：**

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| axisNum | int | 독립 축编号 |

**요청예시：**

```json
{
  "axisNum": 2
}
```

### 독립 축원점 복귀정지

**명령字：** `0x30E6`

**매개변수설명：**

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| axisNum | int | 독립 축编号 |

**요청예시：**

```json
{
  "axisNum": 2
}
```

### 독립 축조그

**명령字：** `0x30E7`

**매개변수설명：**

| 매개변수名 | 유형 | 설명 | 取值범위 |
|--------|------|------|----------|
| axisNum | int | 독립 축编号 | - |
| vel | int | 속도；단위:rpm | - |
| acc | double | 加속도 | - |
| dec | double | 减속도 | - |
| dir | int | 모션方向；1：正，-1：反向 | - |

**요청예시：**

```json
{
  "axisNum": 2,
  "vel": 5,
  "acc": 50.0,
  "dec": 50.0,
  "dir": 1
}
```
