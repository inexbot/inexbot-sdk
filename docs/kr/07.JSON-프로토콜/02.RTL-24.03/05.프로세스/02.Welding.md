...
용접相닫기통신协议主필요포함용접装백업、电流电압력、용접매개변수、摆焊매개변수、용접IO포트、多레이어多道용접、相贯线，닫기于용접프로세스详细소개请参考용접프로세스手册。

---

## 1. 용접装백업

용접装백업설정포함基础기능、微조정정리、防碰撞、再시작/再시작弧와/과其他模块。

### 1.1 호스트 컴퓨터설정용접装백업

**명령어：** `0x8300`

설명：용접过程에 복사합니다，사용하여전송生해제弧、防碰撞、切모드정지等方式해제弧일 때，이기능가능에서해제弧处연결着용접。自이동再시작、半自이동再시작일 때필요반드시해제弧处와/과回退目标위치之사이没有障碍物。

#### basic_functions（基础기능）

| 매개변수名 | 유형 | 설명 | 가져오기값을범위 |
|--------|------|------|----------|
| arc_detection_time | double | 电弧확인일 때사이 | [0,5]s |
| arc_depletion_detection_time | double | 电弧耗尽확인일 때사이 | [0,5]s |
| arc_detection_ensure_time | double | 电弧확인확인일 때사이 | [0,5]s |
| pre_give_gas_time | double | 제공이전전송气일 때사이 | [0,5]s |
| close_time | double | 닫기气일 때사이（음수이(가)제공이전닫기气，양수이(가)延뒤닫기气） | [-5,5]s |
| weld_interruption_detection | bool | 용접에 복사합니다해제확인 | true：에 복사합니다해제확인；false：不에 복사합니다해제확인 |
| arc_fly | bool | 飞줄起弧 | true：飞줄起弧；false：不飞줄起弧 |
| air_close_mode | int | 닫기气方式 | 0: 延迟닫기气，1: 제공이전닫기气 |

#### micro_adjustment（微조정정리）

| 매개변수名 | 유형 | 설명 | 가져오기값을범위 |
|--------|------|------|----------|
| single_adjustment_current | double | 용접电流单회조정정리量 | [0,1000]A |
...
| simulation_set_zero | bool | 灭弧模拟量배치零기능 | true: 열기시작；false：닫기 |
...
반환매개변수同"호스트 컴퓨터설정용접装백업"，参见方表格及예시。
...
| actual_voltage | double[] | 实际电압력 | [0,999]V |
...
| actual_voltage | int[] | 实际电압력 | [0,999]V |
...
### 2.4 호스트 컴퓨터설정전류-전압 매칭매개변수实际값을

**명령어：** `0x8313`

| 매개변수名 | 유형 | 설명 | 가져오기값을범위 |
|--------|------|------|----------|
| type | int | 매개변수유형（电압력1=0, 电압력2=1, 电流1=2, 电流2=3） | [0,3] |
...
|--------|------|------|----------|
| robot | int | 현재로봇 | [1,4] |
| type | int | 선택설정电流/电압력 | 1：설정电流，2：설정电압력 |
| value | double | 값을 | - |
...
| 매개변수名 | 유형 | 설명 | 가져오기값을범위 |
|--------|------|------|----------|
| arcon_use_param | bool | 여부사용起弧매개변수 | true：사용；false：不사용 |
| arcon_current | double | 起弧电流 | [0,1000]A |
| arcon_voltage | double | 起弧电압력 | [-1000,1000]V |
| arcon_time | double | 起弧일 때사이 | [0,5]s |
| arcon_ramp_time | double | 渐변경일 때사이 | [0,1000000]ms |
| arcon_ramp_enable | bool | 起弧渐변경활성화 | - |
| arcon_ramp_mode | int | 起弧渐변경方式 | 0：일 때사이渐변경 |
...
| arcoff_current | double | 수신弧电流 |
| arcoff_time | double | 수신弧일 때사이 |
| arcon_ramp_time | double | 渐변경일 때사이 |
| arcon_ramp_distance | double | 渐변경距离 |
| arcon_ramp_enable | bool | 渐변경활성화 |
| arcon_ramp_mode | int | 수신弧渐변경方式（0：일 때사이渐변경，1：距离渐변경） |
...
| arcon_voltage | double | 起弧电압력 |
| arcon_time | double | 起弧일 때사이 |
| arcon_ramp_time | double | 渐변경일 때사이 |
| arcon_ramp_distance | double | 渐변경距离 |
| arcon_ramp_enable | bool | 起弧渐변경활성화 |
| arcon_ramp_mode | int | 起弧渐변경方式（0：일 때사이渐변경，1：距离渐변경） |
...
inexbot摆焊지원七종류摆이동方式：正弦摆、Z字摆、圆形摆、외부 축설정点摆、L유형摆、三角摆、8字摆。摆이동주파수、摆이동幅도、起시작方向、水平偏角、竖直偏角等매개변수가능조정。
...
| 매개변수名 | 유형 | 설명 | 가져오기값을범위 |
|--------|------|------|----------|
| weaving_type | int | 摆이동유형 | [0,6]，0:正弦，1:Z字形，2:圆形摆，3:외부 축설정点摆，4:L유형摆，5:三角摆，6:8字摆 |
| weaving_freq | int | 摆이동주파수 | (0,20]Hz |
| weaving_ampli | int | 摆이동幅도 | (0,50]mm |
| isMove | int | 여부이동/정지留 | 0：정지留，1：이동 |
| right_standing_time | int | 右정지留일 때사이（선택Z字形摆、외부 축설정点摆、L유형摆、三角摆일 때필요설정） | [0,15] |
| left_standing_time | int | 左정지留일 때사이（선택Z字形摆、외부 축설정点摆、L유형摆、三角摆일 때필요설정） | [0,15] |
| init_direction | int | 起시작方向 | 0：+1，1：-1 |
...
반환매개변수同"호스트 컴퓨터설정摆焊매개변수"，参见方表格及예시。

---

## 5. 용접IO

inexbot제어시스템사용하여숫자化IO포트구현용접프로세스精确제어，구성合模拟信번호인터페이스완료용접매개변수闭环管理。
...
#### Aout（模拟출력，포트 번호부터1시작，0의미없음）

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| given_electric_current | int | 에게설정电流信번호 |
...
반환매개변수同"호스트 컴퓨터설정용접IO인터페이스"，参见方表格及예시。
...
설명：사용자명령어로이명령手이동설정点焊매개변수，포함：电焊电流、点焊电압력、点焊最大일 때사이。

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| spotWeldCurrent | double | 点焊电流 |
| spotWeldVoltage | double | 点焊电압력 |
| spotWeldMaxTime | double | 点焊最大일 때사이 |
...
| 매개변수名 | 유형 | 설명 | 가져오기값을범위 |
|--------|------|------|----------|
| type | int | 焊机모드 | 1：용접활성화，2：手이동起弧，3：手이동点焊，4：전송丝，5：退丝，6：전송气，7：故障리셋 |
...
| monufactor | string | 焊机厂家 | 纳博特、麦格米特、深威智가능、奥太、美佳尼克、瑞凌、肯倍IF15、EWM、福尼斯 |
| weldingMode | int | 焊机工으로모드 | 0：分别，1：一元化 |
| commMode | string | 통신方式 | CAN、ModBus RTU、Modbus TCP |
| slaveIP | string | 슬레이브IP（통신方式이(가)Modbus TCP일 때필요설정） | - |
| slaveid | int | 슬레이브ID（통신方式이(가)ModBus RTU일 때필요설정） | - |
| port | int | 포트 번호（통신方式이(가)ModBus RTU、Modbus TCP일 때필요설정） | (0,65535] |
| baudRate | int | 波特率（통신方式이(가)ModBus RTU일 때필요설정） | - |
| identifiedWelder | string | 완료인식别焊机 | - |
| CANPort | string | CAN통신포트（통신方式이(가)CAN일 때필요설정） | - |
| CANPortSum | int | CAN통신포트数量 | - |
| welderNumber | int | 焊机编번호 | - |
| monufactorList | string[] | 선택择焊机厂家清单 | 纳博特、麦格米特、深威智가능、奥太、美佳尼克、瑞凌、肯倍IF15、EWM、福尼斯、와/과宗 |
...
| 매개변수名 | 유형 | 설명 | 가져오기값을범위 |
|--------|------|------|----------|
| welder[].status | int | 焊机통신상태 | 0：未통신，1：완료통신 |
...
### 9.6 호스트 컴퓨터조회용접매개변수、摆焊매개변수에 복사합니다등록해제
...
| weavParameterNotes | string[] | 摆焊매개변수등록해제 |
| robot | int | 현재로봇 |
...
반환매개변수同"多레이어多道용접매개변수설정"，参见方表格及예시。
...
반환매개변수同"호스트 컴퓨터설정多레이어多道焊道구성"，参见方表格及예시。
...
| weld_parameter_mode | string | 焊机工으로모드 | "一元化"또는"分别" |
...
### 11.3 컨트롤러回복원데이터라이브러리匹구성结果

**명령어：** `0x83A2`

설명：回복원데이터라이브러리에 복사합니다匹구성항목焊道总레이어 수、总道数及매一레이어焊道数。

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| weld_database | array | 匹구성데이터라이브러리 |
| total_layer | int | 제i항목匹구성항목존재焊道总레이어 수 |
| total_bead | int | 제i항목匹구성항목존재焊道总道数 |
| layer | array | 焊道레이어정보 |
| number | int | 제i항목匹구성항목존재제k레이어焊道数 |
...
|--------|------|------|----------|
| workPieceNum | int | 프로세스번호 | [1,3] |
| pointNum | int | 캘리브레이션点（共필요6번째캘리브레이션点） | [0,5] |
...
| workPieceNum | int | 프로세스번호 | [1,3] |
| pointNum | int | 캘리브레이션点（共필요6번째캘리브레이션点） | [0,5] |
| pos | double[] | 해당캘리브레이션点좌표（X,Y,Z,A,B,C） | - |
...
| 매개변수名 | 유형 | 설명 | 가져오기값을범위 |
|--------|------|------|----------|
| result | bool | 여부有계산结果 | true：有，false：没有 |
| workPieceNum | int | 프로세스번호 | [1,3] |
...
### 12.7 컨트롤러回복원相贯线공작물매개변수

**명령어：** `0x83B6`

반환매개변수同"相贯线공작물매개변수설정"，参见方表格及예시。
...
### 12.10 컨트롤러回복원相贯线焊枪매개변수

**명령어：** `0x83B9`

반환매개변수同"相贯线焊枪매개변수설정"，参见方表格及예시。
