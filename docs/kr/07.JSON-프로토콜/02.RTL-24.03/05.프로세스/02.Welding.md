...
용접装备설정包含基础기능、微调整、防碰撞、再启动/再启弧和其他模块。

### 1.1 호스트 컴퓨터설정용접装备

**명령어：** `0x8300`

설명：용접过程中，通过发生断弧、防碰撞、切모드停止等方式断弧时，此기능可在断弧处接着용접。自动再启动、半自动再启动时要确保断弧处和回退目标위치之间没有障碍物。
...
| 매개변수名 | 유형 | 설명 | 取值범위 |
|--------|------|------|----------|
| single_adjustment_current | double | 용접电流单次调整量 | [0,1000]A |
| single_adjustment_voltage | double | 용접电压单次调整量 | [-1000,1000]V |
...
| outputPort | int | 防碰撞상태输出포트 | - |
| outputValue | int | 防碰撞상태输出电平 | 0/1 |
...
| weld_finish_time | double | 용접完成回抽时间 | [0,5]s |
| break_back_enable | bool | 断弧回抽使能 | true: 开启；false：关闭 |
| weld_finish_function | bool | 용접完成回抽기능 | true: 开启；false：关闭 |
| simulation_set_zero | bool | 灭弧模拟量置零기능 | true: 开启；false：关闭 |
...
| AO_setcurrent | int | 电流输出포트 | - |
| AO_setvoltage | int | 电压输出포트 | - |
...
| 매개변수名 | 유형 | 설명 | 取值범위 |
|--------|------|------|----------|
| num | int | 프로세스号（从1开始，可설정1-99个） | [1,99] |
| notes | string | 注释 | - |
| robot | int | 当前로봇 | [1,4] |
...
| 매개변수名 | 유형 | 설명 | 取值범위 |
|--------|------|------|----------|
| arcoff_use_param | bool | 是否使用收弧매개변수 | true：使用；false：不使用 |
...
| 매개변수名 | 유형 | 설명 | 取值범위 |
|--------|------|------|----------|
| arcon_use_param | bool | 是否使用起弧매개변수 | true：使用；false：不使用 |
...
inexbot摆焊支持七种摆动方式：正弦摆、Z字摆、圆形摆、외부 축定点摆、L型摆、三角摆、8字摆。摆动频率、摆动幅度、起始方向、水平偏角、竖直偏角等매개변수可调。
...
inexbot제어시스템通过数字化IO포트구현용접프로세스的精确제어，配合模拟信号인터페이스完成용접매개변수的闭环管理。
...
#### Din（数字输入，포트 번호从1开始，0表示无）

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| arc_success | int | 起弧成功 |
| seek_success | int | 寻位成功 |
| weld_enable | int | 远程용접使能 |
| wire_feed | int | 点动送丝 |
| return_wire | int | 退丝 |
| remote_gastest | int | 气体检测 |

#### Dout（数字输出，포트 번호从1开始，0表示无）
...
| weld_enable | bool | 용접使能 |

#### Ain（模拟输入，포트 번호从1开始，0表示无）
...
| aspiration | bool | 送气 | - |
| returnWire | bool | 退丝 | - |
| pistolSwitch | int | 焊枪开关상태 | 0表示低(红)，1表示高(绿)，-1表示无(灰) |
...
| 매개변수名 | 유형 | 설명 | 取值범위 |
|--------|------|------|----------|
| type | int | 焊机모드 | 1：용접使能，2：手动起弧，3：手动点焊，4：送丝，5：退丝，6：送气，7：故障复位 |
| value | int | 是否开启对应的모드 | 1：开启，0：关闭 |
| robot | int | 当前로봇 | [1,4] |
...
| name | string | 용접电流/电压 | - |
| robot | int | 当前로봇 | [1,4] |
| type | int | 조작유형 | 0：给定值增加，1：给定值减小，2：保存 |
...
| 매개변수名 | 유형 | 설명 | 取值범위 |
|--------|------|------|----------|
| controlMode | int | 제어모드 | 0:模拟제어，1:数字제어 |
...
| 매개변수名 | 유형 | 설명 | 取值범위 |
|--------|------|------|----------|
| finishinit | bool | 컨트롤러初始化完成标志 | true：完成，false：未完成 |
| robot | int | 当前로봇 | [1,4] |
...
| 매개변수名 | 유형 | 설명 | 取值범위 |
|--------|------|------|----------|
| num_source | int | 复制的프로세스号 | - |
| num_target | int | 复制到的프로세스号 | - |
| robot | int | 当前로봇 | [1,4] |
...
|--------|------|------|----------|
| workPieceNum | int | 프로세스号 | [1,3] |
| pointNum | int | 캘리브레이션点（共需要6个캘리브레이션点） | [0,5] |
...
### 12.7 컨트롤러回复相贯线工件매개변수

**명령어：** `0x83B6`

반환매개변수同"相贯线工件매개변수설정"，参见上方表格及예시。
