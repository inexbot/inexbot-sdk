...
용접相닫기통신协议主필요포함용접装백업、电流电압력、용접매개변수、위빙매개변수、용접IO포트、多레이어多道용접、相贯线，닫기于용접프로세스详细소개请参考용접프로세스手册。
...
| air_close_mode | int | 닫기气방식 | 0: 延迟닫기气，1: 제공이전닫기气 |
...
| actual_current | double[] | 실제电流 | [0,999]A |
...
| actual_current | int[] | 실제电流 | [0,999]A |
...
| arcon_ramp_mode | int | 起弧渐변경방식 | 0：일 때사이渐변경 |
...
| arcon_ramp_mode | int | 수신弧渐변경방식（0：일 때사이渐변경，1：距离渐변경） |
...
| arcon_ramp_distance | double | 渐변경距离 |
| arcon_ramp_enable | bool | 起弧渐변경활성화 |
| arcon_ramp_mode | int | 起弧渐변경방식（0：일 때사이渐변경，1：距离渐변경） |
...
inexbot위빙지원七종류摆이동방식：正弦摆、Z字摆、圆形摆、외부 축설정点摆、L유형摆、三角摆、8字摆。摆이동주파수、摆이동幅도、起시작방향、水平偏角、竖直偏角等매개변수가능조정。
...
반환매개변수同"호스트 컴퓨터설정위빙매개변수"，参见方表格及예시。
...
설명：사용자명령어로이명령手이동설정点焊매개변수，포함：电焊电流、点焊电압력、点焊최대일 때사이。

| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| spotWeldCurrent | double | 点焊电流 |
| spotWeldVoltage | double | 点焊电압력 |
| spotWeldMaxTime | double | 点焊최대일 때사이 |
...
| baudRate | int | 波特率（통신방식이(가)ModBus RTU일 때필요설정） | - |
