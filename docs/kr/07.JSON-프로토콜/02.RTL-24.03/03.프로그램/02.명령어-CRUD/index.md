...
- "cmd":该节点为명령어的具体信息，详情查看명령어页面
- 호스트 컴퓨터下发삽입명령어
**명령어:** `0x5200`
- "robot":1
- 表示要操作的로봇 번호；int유형，取值범위[1,4]
- "pos":2
- 表示삽입的줄号；int유형
- "savejobfile":false
...
- 예아니오수정평활；bool유형
- "PL_VALUE_":"3"
- 평활；string유형；取值범위[0,5]
- "ACC_ENABLE_":true
- 예아니오수정가속도；bool유형
- "ACC_VALUE_":"50"
- 가속도；string유형；取值범위[1,100]
- "DEC_ENABLE_":true
- 예아니오수정감속도；bool유형
- "DEC_VALUE_":"50"
- 감속도；string유형；取值범위[1,100]
- "TEC_TYPE_":1
- 매개변수；int유형；取值범위[0,10]
- 0:网络通信클래스；1：팔레타이징제어클래스；2：비전命令클래스；3：레이저클래스；4：컨베이어클래스
- 5：그라인딩프로세스클래스；6：용접매개변수；7：摆焊매개변수；8：相贯线；9：寻件文件号；10：추적文件号
- "TEC_ENABLE_":true
- 예아니오수정相关유형프로세스号；bool유형
- "TEC_VALUE_":"5"
- 相关유형프로세스号；string유형；取值범위与相关유형有关
- "savejobfile":false
- 예아니오저장文件；bool유형
- true저장，false不저장
- 当选择false时可以使用0x5120저장작업 파일
...
- "store":true
- 예아니오存储현재수정
- 当"store"존재，并且值为true时존재"time","savejobfile"节点
- "time":"2020.02.26 10:11:06"
- 타임스탬프；string유형
- "savejobfile":false
- 예아니오저장文件；bool유형
