...
| 매개변수名 | 유형 | 取值범위 | 설명 |
|--------|------|----------|------|
| mode | int | [0,3] | 드래그方式：0-없음，1-3D鼠标，2-토크，3-위치 |
...
| drag_mode | int | [0,2] | 드래그모드：0-自由드래그，1-위치드래그，2-姿态드래그 |
| frictionOffset | double[6] | [0,0.99] | 관절摩擦力补偿矫正系数 |
| targetTorqCoefficient | double[6] | [0,1] | 관절目标扭矩矫正系数 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| TrajName | string[] | 已저장궤적열表，길이为현재已저장궤적数量，若없음궤적则该节点존재하지 않음 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| absolutepath | string | 컨트롤러中拖拽궤적파일存放주소 |
| trajectory_filelist | string[] | 拖拽궤적파일열表，"drag_traj_details.json"为궤적 기록信息파일 |
