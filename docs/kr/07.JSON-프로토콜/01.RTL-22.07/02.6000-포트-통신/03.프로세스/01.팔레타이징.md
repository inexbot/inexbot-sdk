...
![설정抓手매개변수0x4201 PAL_GRIPPER_PARM_SET的JSON데이터格式](assets/json-23.png)
...
| robot | int | 是 | 로봇 번호 |
| craftID | int | 是 | 프로세스号 |
| gripperNum | int | 否 | 抓手个数，범위1~4 |
| gripper | array | 否 | 抓手도구号数组，범위1~9 |
...
| robot | int | 로봇 번호 |
| craftID | int | 프로세스号 |
| gripperNum | int | 抓手个数，범위1~4 |
| gripper | array | 抓手도구号数组，범위1~9 |
...
![설정托盘매개변수0x4204 PAL_PALLET_PARM_SET的JSON데이터格式](assets/json-24.png)
...
![설정위치매개변수0x4207 PAL_POS_PARM_SET的JSON데이터格式](assets/json-25.png)
...
![설정工件매개변수0x420A PAL_WORKPIECE_PARM_SET的JSON데이터格式](assets/json-26.png)
...
| workpieceApproMode | int | 否 | 接近모드：0-下降接近，1-接近下降 |
...
![설정重叠모드매개변수0x4210 PAL_OVERLAP_PARM_SET的JSON데이터格式](assets/json-27.png)
...
| craftID | int | 是 | 프로세스号 |
| floorSum | int | 否 | 层数 |
| overlapType | int | 否 | 重叠유형：0-相同，1-交替，2-自定义 |
...
| fixedEnterPoint | bool | 否 | 固定入口点위치 |
...
![설정平面모드매개변수0x4213 PAL_PLANE_PARM_SET的JSON데이터格式](assets/json-28.png)
...
| craftID | int | 是 | 프로세스号 |
| graphic | int | 是 | 图像编号 |
| graphicType | int | 是 | 图形유형：0-行列，1-纵横交错，2-回字型，3-五花垛，4-自定义 |
...
| craftID | int | 是 | 프로세스号 |
| graphic | int | 是 | 图像编号 |
| graphicType | int | 是 | 图形유형（4-自定义） |
| transLenX | double | 否 | X平移补偿 |
| transLenY | double | 否 | Y平移补偿 |
| count | int | 是 | 层工件数 |
| sum | int | 是 | 层工件总数 |
| start | int | 是 | 起始编号 |
| eachVec | array | 是 | 每层데이터 |
...
![설정팔레타이징상태0x421A PAL_STATE_SET的JSON데이터格式](assets/json-29.png)
...
![复制팔레타이징매개변수0x421D PAL_PARM_COPY的JSON데이터格式](assets/json-30.png)
...
|--------|------|------|------|
| robot | int | 是 | 로봇 번호 |
| craftID_source | int | 是 | 源프로세스号 |
| craftID_target | int | 是 | 目标프로세스号 |
...
![复制层图形매개변수0x421F PAL_GRAPHIC_COPY的JSON데이터格式](assets/json-31.png)
...
![切换使用팔레타이징유형0x4221 PAL_SIMPLE_SWTICH的JSON데이터格式](assets/json-32.png)
...
| robot | int | 是 | 로봇 번호 |
| craftID | int | 是 | 프로세스号 |
| usePalletType | int | 是 | 팔레타이징유형：0-简易，1-完整，2-未定义 |
...
![简易팔레타이징0x4224 PAL_SIMPLE_POS_SET的JSON데이터格式](assets/json-33.png)
...
![팔레타이징复位0x422C PAR_PARM_RESET的JSON데이터格式](assets/json-34.png)
...
| L | double | 工件长度 |
| W | double | 工件宽度 |
| over | int | 超限상태：0-无超限，1-入口点超限，2-辅助点超限，3-工件点超限 |
| overLayer | int | 超限层号 |
| overNum | int | 超限工件号 |
| sum | int | 层工件总数 |
| count | int | 当前层工件数 |
...
![点位调试界面반환全部工件데이터0x4243 PAL_POINTDEBUG_FLOORDATA_RESPOND的JSON데이터格式](assets/json-35.png)
...
| layer | int | 是 | 层号 |
| num | int | 是 | 工件编号 |
| mode | int | 是 | 모드：0-直接설정xyz，1-使用로봇当前위치 |
| X | double | 否 | 工件X좌표 |
| Y | double | 否 | 工件Y좌표 |
| Z | double | 否 | 工件Z좌표 |
| angle | double | 否 | 工件角度 |
...
| layer | int | 是 | 层号 |
| num | int | 是 | 工件编号 |
| type | int | 是 | 点位유형：0-入口点，1-辅助点，2-工件点 |
