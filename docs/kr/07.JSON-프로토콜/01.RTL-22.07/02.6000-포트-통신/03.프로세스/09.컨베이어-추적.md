...
| conveyor.posRecordMode | int | 컨베이어위치모드：0-编码器；1-恒速설정 |
| conveyor.speed | double | 当컨베이어위치모드为恒速설정时的속도 |
| conveyor.userCoord | int | 사용자좌표계1~9 |
| track.height | int | 追踪目标高度：0-传感器感知；1-跟踪명령어示教 |
| track.track_on_run_mode_with_target_overrun | int | 目标超限실행方式：0-等待下一个目标；1-跳行到跟踪结束실행 |
...
| conveyorID | int | 컨베이어프로세스号 |
| robot | int | 로봇 번호 |
| detectSrc.type | int | 工件检测信号源：0-비전；1-IO；2-全局변수 |
| detectSrc.globalVar | string | 当工件检测信号源为2-全局변수时生效 |
...
| detectSrc.vision_latch_encoder_value_type | int | 비전锁存编码器值：0-触发锁存；1-收到데이터锁存 |
| identification.communication | int | 비전통신方式：0-以太网；1-Modbus |
| identification.sensorTrg | int | 传感器触发方式：0-低电平触发；1-高电平触发 |
| identification.type | int | 工件识别方式：0-비전；1-传感器 |
...
| conveyorID | int | 컨베이어프로세스号 |
| robot | int | 로봇 번호 |
| conveyor.encoderValue | double | 编码器值（int64유형） |
...
| userFrame | array | 弧度制사용자좌표계 |
| userFrameDeg | array | 角度制사용자좌표계 |
...
使用0x4811캘리브레이션3次后0x4810计算사용자좌표계，会且반환0x3C02{"result":true}查看캘리브레이션是否成功
...
| robot | int | 로봇 번호 |
| conveyorID | int | 컨베이어프로세스号 |
| posNum | int | 캘리브레이션次数（캘리브레이션3次：1,2,3） |
...
| posX | double | x좌표 |
| posY | double | y좌표 |
| encodorValue | int | 编码器数值（int64유형） |
...
> 注：使用비전识别工件则无需캘리브레이션，直接跳过。传感器则0x4804需要设为传感器和传感器触发的方式。
...
| robot | int | 로봇 번호 |
| conveyorID | int | 컨베이어프로세스号 |
| sensorPos | object | 弧度制传感器위치 |
| sensorPosDeg | object | 角度制传感器위치 |
...
| sensorCalibration.IO_encodorValue | int | 传感器编码器数值（int64유형） |
| sensorCalibration.IO_time | int | IO时间 |
| sensorCalibration.calib_X | double | 캘리브레이션点UX |
| sensorCalibration.calib_Y | double | 캘리브레이션点UY |
| sensorCalibration.calib_encodorCircles | int | 캘리브레이션编码器圈数 |
| sensorCalibration.calib_encodorValue | int | 캘리브레이션点编码器数值（int64유형） |
...
| position.trackRangeXMax | double | 跟踪범위X最大 |
| position.trackRangeYMin | double | 跟踪범위Y最小 |
| position.trackRangeYMax | double | 跟踪범위Y最大 |
| position.trackRangeZMin | double | 跟踪범위Z最小 |
| position.trackRangeZMax | double | 跟踪범위Z最大 |
...
> 注：반환데이터的position매개변수里多了posToMove是0x4833캘리브레이션点
