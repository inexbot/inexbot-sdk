...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| produce_num | int | 当前실행次数 |
| target_num | int | 目标실행次数 |
| waste_num | int | 废料数目 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| punch_enable | bool | 스탬핑使能 |
| run_without_workpiece_enable | bool | 无料실행开关 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| online_status_reset | int | 联机상태复位명령（固定为1） |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| function_num | int | 기능号：0启动，1暂停，2停止，3回到待机点后暂停，4清料流程 |
...
| communition_status | bool | 통신상태：false통신异常，true통신正常 |
| robot_sum | int | 로봇总数：0，1，2，3 |
| current_robot_num | int | 当前로봇 번호：1，2，3 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| type | int | 유형：0取料失败，1放料失败1，2放疗失败2，3首模完成 |
| open_close | int | 조작：0关闭弹窗，1弹出弹窗 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| type | int | 유형：0取料失败，1放料失败，2스탬핑失败，3人工处理，5有无料启动 |
| deal_method | int | 处理方式：0重新放料/重新放料/人工스탬핑/人工스탬핑，1放弃本料/放弃本料/继续실행/继续실행 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| current_chosed_craft_num | int | 选中当前프로세스号（1，2，3，4） |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| current_chosed_craft_num | int | 当前选中프로세스号 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| current_chosed_craft_num | int | 当前选中프로세스号（1，2，3，4） |
...
| interp_mode | int | 模内逃跑插补方式：0관절，1直线 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| cummunity_status | int | 통신상태：0통신异常，1통신正常 |
| robot_sum | int | 로봇总数 |
| current_robot_num | int | 当前로봇 번호 |
...
| punch_singal_param | int | 스탬핑信号매개변수：0上死点复位，1延迟复位 |
| delay_reset_time | int | 延迟复位时间 |
| punch_timeout_time | int | 스탬핑超时설정 |
| react_timeout_time | int | 反应超时설정 |
| punch_success_confirm_time | int | 스탬핑成功确认时间 |
...
| control_punch_enable | int | 允许제어스탬핑：0 off，1 on |
| puncher_type | int | 冲床유형：0，1，2，3 |
| punch_singal_param | int | 스탬핑信号매개변수：0延迟复位，1上死点复位 |
| delay_reset_time | int | 延迟复位时间 |
| punch_timeout_time | int | 스탬핑超时설정 |
| react_timeout_time | int | 反应超时설정 |
| punch_success_confirm_time | int | 스탬핑成功确认时间 |
...
| punch_singal_param | int | 스탬핑信号매개변수：1上死点复位，0延迟复位 |
| delay_reset_time | int | 延迟复位时间 |
| punch_timeout_time | int | 스탬핑超时설정 |
| react_timeout_time | int | 反应超时설정 |
| punch_success_confirm_time | int | 스탬핑成功确认时间 |
...
| request_call_feed_timeout | int | 呼叫投料超时설정 |
...
> 설명：数组第0位为port（범위0-16），数组第1位为value（범위0或1）
...
| prev_once_punch_check | 前站单次스탬핑检测 |
| local_once_punch_check | 本站单次스탬핑检测 |
...
| prev_station_interfere_zone | 前站간섭 영역输出 |
| local_station_interfere_zone | 本站간섭 영역输出 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| modify_port | int | 辅助输出포트 번호（1.2.3.4...） |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| interfere_zone_enable | int | 간섭 영역使能（0或1） |
...
| rFx_Zero_without_load_GD_num | int | 无负载Fx영점GD编号 |
| rFy_Zero_without_load_GD_num | int | 无负载Fy영점GD编号 |
| rFz_Zero_without_load_GD_num | int | 无负载Fz영점GD编号 |
| rTx_Zero_without_load_GD_num | int | 无负载Tx영점GD编号 |
| rTy_Zero_without_load_GD_num | int | 无负载Ty영점GD编号 |
| rTz_Zero_without_load_GD_num | int | 无负载Tz영점GD编号 |
...
| force_sensor_sign_value_without_load_data_time | int | 空载力矩标记데이터时间（ms） |
| force_sensor_sign_value_with_load_data_time | int | 带载力矩标记데이터时间（ms） |
| force_sensor_sign_value_without_load_data_range | float | 空载力矩标记데이터범위 |
| force_sensor_sign_value_with_load_data_range | float | 带载力矩标记데이터범위 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| already_sign_standard_pos | bool | 是否已标记床板水平위치 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| auto_modify_acc_enable | bool | 自适应加减속도使能 |
| arm_inertia | array | 臂惯性 |
| joint1_mass | float | 관절1质量 |
| joint2_mass | float | 관절2质量 |
| joint1_mass_center_to_motor_distance | float | 관절1质心到电机距离 |
| joint2_mass_center_to_motor_distance | float | 관절2质心到电机距离 |
