...
| prev_station_work_condition | int | 前站料况：0없음料，1有料已冲，2有料未冲 |
| local_device_num | int | 本站设备号 |
| local_station_work_condition | int | 本站料况：0없음料，1有料已冲，2有料未冲 |
| fixture_num | int | 治具号 |
| fixture_station_work_condition | int | 治具料况：0없음料，1有料已冲，2有料未冲 |
...
| run_without_workpiece_enable | bool | 없음料실행开关 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| single_run_test | int | 单机试실행명령（固定为1） |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| escape_from_mould | int | 模内逃跑测试명령（固定为1） |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| online_status_reset | int | 联机상태리셋명령（固定为1） |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| function_num | int | 기능号：0시작，1일시정지，2정지，3回到待机点后일시정지，4清料流程 |
...
| local_puncher_num | int | 本站冲床编号 |
| run_cycle | int | 실행주기 |
| fixture_check | bool | 治具检测：true不检测，false정상检测 |
| prev_station_type | int | 前站유형：0上料机，1放料台，2冲床，3翻转台，4空中接力 |
| online_work_type | int | 联机工비트：0单机，1上料，2搬运，3下料 |
| local_station_type | int | 本站유형：0上料机，1放料台，2冲床，3翻转台，4空中接力 |
...
| local_punch_success | bool | 本站冲床성공：false off，true on |
| prev_punch_success | bool | 前站冲床성공：false off，true on |
| local_workpiece_condition | int | 本站料况：0없음料，1有料未冲，2有料已冲 |
| prev_workpiece_condition | int | 前站料况：0없음料，1有料未冲，2有料已冲 |
| local_in_interfere_zone | bool | 后站在간섭 영역：false off，true on |
| prev_in_interfere_zone | bool | 前站在간섭 영역：false off，true on |
| in_prev_interfere_zone | bool | 在前站간섭 영역：false off，true on |
| in_local_interfere_zone | bool | 在本站간섭 영역：false off，true on |
| fixture_workpece_condition | int | 抓手料况：0없음料，1有料未冲，2有料已冲 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| type | int | 유형：0取料실패，1放料실패1，2放疗실패2，3首模완료 |
| open_close | int | 조작：0닫기弹窗，1弹出弹窗 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| type | int | 유형：0取料실패，1放料실패，2스탬핑실패，3人工处理，5有없음料시작 |
| deal_method | int | 处理方式：0重新放料/重新放料/人工스탬핑/人工스탬핑，1放弃本料/放弃本料/계속실행/계속실행 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| current_chosed_craft_num | int | 选中현재프로세스号（1，2，3，4） |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| current_chosed_craft_num | int | 현재选中프로세스号 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| current_chosed_craft_num | int | 현재选中프로세스号（1，2，3，4） |
...
| interp_mode | int | 模内逃跑보간方式：0관절，1直线 |
| escape_speed_percent | int | 模内逃跑속도比（1-100） |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| online_station_type | int | 联机工비트：0单机，1上料，2搬运，3下料 |
| next_IP | string | 下站IP |
...
| dout_fetch_finish | int | 출력取料완료：0 off，1 on |
...
| punch_time | int | 스탬핑时长 |
| puncher_type | int | 冲床유형：0，1，2，3 |
| punch_singal_param | int | 스탬핑信号매개변수：0上死点리셋，1延迟리셋 |
| delay_reset_time | int | 延迟리셋时间 |
| punch_timeout_time | int | 스탬핑타임아웃설정 |
| react_timeout_time | int | 反应타임아웃설정 |
| punch_success_confirm_time | int | 스탬핑성공确认时间 |
| punch_mould_check | int | 스탬핑模内检测：0 off，1 on |
| top_point_check_when_start | int | 시작时上死点检测：0 off，1 on |
...
| control_punch_enable | int | 允许제어스탬핑：0 off，1 on |
| puncher_type | int | 冲床유형：0，1，2，3 |
| punch_singal_param | int | 스탬핑信号매개변수：0延迟리셋，1上死点리셋 |
| delay_reset_time | int | 延迟리셋时间 |
| punch_timeout_time | int | 스탬핑타임아웃설정 |
| react_timeout_time | int | 反应타임아웃설정 |
| punch_success_confirm_time | int | 스탬핑성공确认时间 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| local_station_type | int | 本站유형：1放料台，2冲床，3翻转台，4空中接力 |
| puncher_num | int | 冲床编号 |
| puncher_sum | int | 最大冲床数 |
| puncher_type | int | 冲床유형：0齿轮，1气动，2油压，3自定义 |
| punch_singal_param | int | 스탬핑信号매개변수：1上死点리셋，0延迟리셋 |
| delay_reset_time | int | 延迟리셋时间 |
| punch_timeout_time | int | 스탬핑타임아웃설정 |
| react_timeout_time | int | 反应타임아웃설정 |
| punch_success_confirm_time | int | 스탬핑성공确认时间 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| feeder_signal_type | int | 投料机信号유형：0上升沿，1常通信号，2不检测 |
...
> 설명：배열제0비트为port（범위0-16），배열제1비트为value（범위0或1）
...
| prev_station_puncher_emergency_stop | 冲床비상 정지前站 |
| local_station_puncher_emergency_stop | 冲床비상 정지本站 |
| puncher_check_mould | 冲床模内检测 |
| prev_once_punch_check | 前站单회스탬핑检测 |
| local_once_punch_check | 本站单회스탬핑检测 |
...
| 매개변수名 | 설명 |
|--------|------|
| prev_station_punch_signal | 스탬핑信号前站 |
| local_station_punch_signal | 스탬핑信号本站 |
| emergency_stop_prev_station_puncher | 비상 정지冲床前站 |
| emergency_stop_local_station_puncher | 비상 정지冲床本站 |
| fixture1 | 治具1 |
| fixture2 | 治具2 |
| request_feed | 요청投料 |
| turn_table_fetch_success | 翻转台取料성공 |
| turn_table_lay_success | 翻转台放料성공 |
| prev_station_interfere_zone | 前站간섭 영역출력 |
| local_station_interfere_zone | 本站간섭 영역출력 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| interfere_zone_sign | string | 标记간섭 영역：prev_interfere_zone（前站冲床），local_interfere_zone（本站冲床） |
...
|--------|------|------|
| interfere_zone_sign | string | 标记간섭 영역：prev_interfere_zone（前站冲床），local_interfere_zone（本站冲床） |
| prev_station_interfere_zone_X | float | 前站간섭 영역X좌표 |
| prev_station_interfere_zone_Y | float | 前站간섭 영역Y좌표 |
| local_station_interfere_zone_X | float | 本站간섭 영역X좌표 |
| local_station_interfere_zone_Y | float | 本站간섭 영역Y좌표 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| interfere_zone_enable | int | 간섭 영역활성화（0或1） |
| prev_station_x_min | float | 前站간섭 영역x最小 |
| prev_station_x_max | float | 前站간섭 영역x最大 |
| prev_station_y_min | float | 前站간섭 영역y最小 |
| prev_station_y_max | float | 前站간섭 영역y最大 |
| local_station_x_min | float | 本站간섭 영역x最小 |
| local_station_x_max | float | 本站간섭 영역x最大 |
| local_station_y_min | float | 本站간섭 영역y最小 |
| local_station_y_max | float | 本站간섭 영역y最大 |
...
| pallet_type | int | 팔레타이징유형：0前站팔레타이징，1本站팔레타이징 |
| pallet_ID | int | 팔레타이징프로세스号 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| chosed_pos_num | int | 포인트编号：1取料待机，2取料上方，3取料点，4放料待机，5放料上方，6放料点 |
...
|--------|------|------|
| chosed_pos_num | int | 포인트编号：1取料待机，2取料上方，3取料点，4放料待机，5放料上方，6放料点 |
| pos_value | array | 위치좌표值 |
| ext_pos_value | array | 외부 축위치좌표值 |
...
| reset_delay_time | array | 리셋延时 |
| trigger_puncher_num | array | 触发冲床号 |
| reset_puncher_num | array | 리셋冲床号 |
| port | array | IO포트 |
| value | array | 유효值 |
| enable | array | 활성화开关 |
...
```json
{
  ...
  "stage": "本站放料前处理",
  "judge": [
    {"后机不在간섭 영역": true, "放料迟滞时间": false, "本站在上死点": true},
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| detect_enable | bool | 检测활성화 |
| switch_modify_value_by_recorded | bool | 按记录值수정开关 |
| back_off_distance | float | 回退距离 |
| back_off_vel | float | 回退속도 |
| back_off_acc | float | 回退加속도 |
...
| rFx_Zero_without_load_GD_num | int | 없음负载Fx영점GD编号 |
| rFy_Zero_without_load_GD_num | int | 없음负载Fy영점GD编号 |
| rFz_Zero_without_load_GD_num | int | 없음负载Fz영점GD编号 |
| rTx_Zero_without_load_GD_num | int | 없음负载Tx영점GD编号 |
| rTy_Zero_without_load_GD_num | int | 없음负载Ty영점GD编号 |
| rTz_Zero_without_load_GD_num | int | 없음负载Tz영점GD编号 |
| rFx_Zero_with_load_GD_num | int | 带负载Fx영점GD编号 |
| rFy_Zero_with_load_GD_num | int | 带负载Fy영점GD编号 |
| rFz_Zero_with_load_GD_num | int | 带负载Fz영점GD编号 |
| rTx_Zero_with_load_GD_num | int | 带负载Tx영점GD编号 |
| rTy_Zero_with_load_GD_num | int | 带负载Ty영점GD编号 |
| rTz_Zero_with_load_GD_num | int | 带负载Tz영점GD编号 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| sign_value_without_load | array | 空载토크值 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| sign_value_with_load | array | 带载토크值 |
...
| force_sensor_sign_value_without_load_data_time | int | 空载토크标记데이터时间（ms） |
| force_sensor_sign_value_with_load_data_time | int | 带载토크标记데이터时间（ms） |
| force_sensor_sign_value_without_load_data_range | float | 空载토크标记데이터범위 |
| force_sensor_sign_value_with_load_data_range | float | 带载토크标记데이터범위 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| already_sign_standard_pos | bool | 여부已标记床板水平위치 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| payload_enable | bool | 负载활성화 |
| axis1_rated_torq | float | 축1额定扭矩 |
| axis2_rated_torq | float | 축2额定扭矩 |
| axis3_rated_torq | float | 축3额定扭矩 |
| axis4_rated_torq | float | 축4额定扭矩 |
| axis5_rated_torq | float | 축5额定扭矩 |
| axis6_rated_torq | float | 축6额定扭矩 |
| payload_num | int | 负载编号（1-10） |
| payload_mass | float | 负载质量 |
| payload_X | float | 负载X좌표 |
| payload_Y | float | 负载Y좌표 |
| payload_Z | float | 负载Z좌표 |
| payload_name | string | 负载이름 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| auto_modify_acc_enable | bool | 自适应加减속도활성화 |
| arm_inertia | array | 臂惯性 |
| joint1_mass | float | 관절1质量 |
| joint2_mass | float | 관절2质量 |
| joint1_mass_center_to_motor_distance | float | 관절1质心到모터距离 |
| joint2_mass_center_to_motor_distance | float | 관절2质心到모터距离 |
