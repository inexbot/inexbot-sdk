...
![설정激光器매개변수0x4130 TRACK_LASER_PARAM_SET的JSON데이터格式](assets/json-09.png)

설정激光器구성매개변수

![설정激光器구성매개변수0x4130 TRACK_LASER_PARAM_SET的JSON데이터格式](assets/json-10.png)
...
| 매개변수名 | 유형 | 必填 | 설명 |
|--------|------|------|------|
| fileNum | int | 是 | 激光器파일号 |
| robot | int | 是 | 로봇 번호 |
| laserParam.communication | int | 是 | 통신方式：0-modbus tcp，1-ethernet tcp |
| laserParam.devid | int | 是 | 设备号 |
| laserParam.ip | string | 是 | IP주소 |
| laserParam.port | int | 是 | 포트 번호 |
| laserParam.responseTimeout | float | 否 | 응답超时 |
...
| fileNum | int | 激光器파일号 |
| robot | int | 로봇 번호 |
| laserParam.commLog | bool | 통신日志 |
| laserParam.communication | int | 통신方式：0-modbus tcp，1-ethernet tcp |
| laserParam.devid | int | 设备号 |
| laserParam.ip | string | IP주소 |
| laserParam.netstate | bool | 网络상태 |
| laserParam.port | int | 포트 번호 |
| laserParam.responseTimeout | float | 응답超时 |
...
![激光器캘리브레이션0x4140 SENSOR_LASER_CALIBRATE_INQUIRE的JSON데이터格式](assets/json-11.png)
...
| robot | int | 로봇 번호 |
| fileNum | int | 激光器파일号 |
| recordResult.point1~7 | bool | 各캘리브레이션点记录结果 |
...
| robot | int | 是 | 로봇 번호 |
| fileNum | int | 是 | 激光器파일号 |
| pointNum | int | 是 | 캘리브레이션点编号(범위1~7) |
...
| robot | int | 로봇 번호 |
| fileNum | int | 激光器파일号 |
| laserCalibrated | bool | 是否已캘리브레이션 |

---

## 설정寻位유형

![설정寻位유형0x4133 LOCATING_SENSORTYPE_SET的JSON데이터格式](assets/json-12.png)
...
| fileNum | int | 是 | 激光器파일号 |
| robot | int | 是 | 로봇 번호 |
| sensorType | int | 是 | 寻位유형：0-线激光，1-电弧 |
...
| robot | int | 로봇 번호 |
| fileNum | int | 激光器파일号 |
| sensorType | int | 寻位유형：0-线激光，1-电弧 |

---

## 설정跟踪유형

![설정跟踪유형0x4169 TRACK_SENSORTYPE_SET的JSON데이터格式](assets/json-13.png)
...
| robot | int | 是 | 로봇 번호 |
| fileNum | int | 是 | 激光器파일号 |
| sensorType | int | 是 | 跟踪유형：0-线激光，1-电弧，2-弧压 |
...
![설정激光跟踪매개변수表0x4136 TRACK_LASER_TRACKPARAM_SET的JSON데이터格式](assets/json-14.png)
...
| trackParam.positionHold.switchon | bool | 否 | 寻位保存기능：true开启，false关闭 |
| trackParam.scanErrorLength | float | 否 | 扫描错误确认距离 |
| trackParam.sensitivity | int | 否 | 灵敏度 |
| trackParam.trackMode | int | 否 | 跟踪모드：0-绝对式，1-定点增量式，2-行走增量式 |
...
![寻位매개변수表0x4139 TRACK_LASER_SEARCHPARAM_SET的JSON데이터格式](assets/json-15.png)
...
| searchParam.dynamic.speed | float | 否 | 动态寻位속도 |
| searchParam.laserTaskId | int | 否 | 激光器任务号 |
| searchParam.storeType | int | 否 | 寻位유형：0-基准寻位，1-修正寻位 |
...
![复制寻位매개변수0x413C TRACK_SEAMTRACK_PARAM_COPY的JSON데이터格式](assets/json-16.png)

复制寻位매개변수（复制前要0x413D清空）
...
| 매개변수名 | 유형 | 必填 | 설명 |
|--------|------|------|------|
| dstFileNum | int | 是 | 复制到的파일号 |
| fileNum | int | 是 | 复制的파일号 |
| function | int | 是 | 0-跟踪，1-寻位 |
| robot | int | 是 | 로봇 번호 |
| sensorType | int | 是 | 跟踪유형：0-线激光，1-电弧，2-弧压；寻位유형：0-线激光，1-电弧 |
...
![电弧跟踪프로세스0x4150 TRACK_ARC_COMMPARAM_SET的JSON데이터格式](assets/json-17.png)

설정통신매개변수

![설정통신매개변수0x4150 TRACK_ARC_COMMPARAM_SET的JSON데이터格式](assets/json-18.png)
...
| robot | int | 是 | 로봇 번호 |
| craftid | int | 是 | 跟踪파일号 |
| sampling.dataType | int | 是 | 데이터유형：0-电流，1-电压 |
...
| robot | int | 로봇 번호 |
| craftid | int | 跟踪파일号 |
| sampling.dataType | int | 데이터유형：0-电流，1-电压 |
...
![설정左右补偿매개변수0x4153 TRACK_ARC_LATERALCOMPENPARAM_SET的JSON데이터格式](assets/json-19.png)
...
| craftid | int | 是 | 跟踪파일号 |
| robot | int | 是 | 로봇 번호 |
| lateralCompensation.accFactor | float | 否 | 补偿加속도倍数(0.1~10) |
| lateralCompensation.algorithmType | int | 否 | 偏差提取유형：0-均值 |
...
![설정高低补偿매개변수0x4156 TRACK_ARC_VERTICALCOMPENPARAM_SET的JSON데이터格式](assets/json-20.png)
...
| craftid | int | 是 | 跟踪파일号 |
| robot | int | 是 | 로봇 번호 |
| verticalCompensation.accFactor | float | 否 | 补偿加속도倍数 |
| verticalCompensation.algorithmType | int | 否 | 偏差提取유형 |
...
![触摸寻位매개변수0x4160 SEARCH_TOUCH_PARAM_SET的JSON데이터格式](assets/json-21.png)
...
| craftid | int | 是 | 寻位파일号 |
| robot | int | 是 | 로봇 번호 |
| touchSearch.2ndAutoDistance | float | 否 | 二次自动반환距离 |
| touchSearch.2ndAutoReturn | bool | 否 | 二次自动반환使能 |
| touchSearch.2ndAutoVel | float | 否 | 二次自动반환속도 |
| touchSearch.2ndDeviationLimit | float | 否 | 二次超偏差범위 |
| touchSearch.2ndDistance | float | 否 | 二次寻位距离 |
| touchSearch.2ndSwitchon | bool | 否 | 二次寻位使能 |
| touchSearch.2ndVel | float | 否 | 二次寻位속도 |
| touchSearch.autoDistance | float | 否 | 自动반환距离 |
| touchSearch.autoReturn | bool | 否 | 自动반환使能 |
| touchSearch.autoVel | float | 否 | 自动반환속도 |
| touchSearch.baseFlag | bool | 否 | 基准寻位开关 |
| touchSearch.compensation | float | 否 | 모션向量补偿 |
| touchSearch.deviationLimit | float | 否 | 超偏差범위 |
| touchSearch.distance | float | 否 | 寻位距离 |
| touchSearch.isChangePose | bool | 否 | 是否变姿态 |
| touchSearch.vel | float | 否 | 寻位속도 |
...
![弧压追踪매개변수0x4163 ARC_VOLTAGE_TRACK_PARAMETERS_SET的JSON데이터格式](assets/json-22.png)
...
| craftid | int | 是 | 跟踪파일号 |
| robot | int | 是 | 로봇 번호 |
| base_calc.collect_time | float | 否 | 용접开始计算时间(s) |
| base_calc.method | int | 否 | 基准电压获取方式：0-용접计算，1-手动计算 |
| base_calc.vol_inc | float | 否 | 计算增量 |
| base_calc.voltage | float | 否 | 基准电压(=计算量+计算增量) |
| collection.analog_port | int | 否 | 弧压采集模拟포트(AIN-1...) |
| collection.equipment | int | 否 | 弧压采集设备：0-焊机，1-弧压模块 |
| collection.invalid_data_time | float | 否 | 无效데이터时间(s) |
