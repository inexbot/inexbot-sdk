...
| vision.trigger | IOPort | int | 触전송方式 绑설정IO포트 |
...
| 매개변수分클래스 | 매개변수名 | 유형 | 설명 |
|---------|--------|------|------|
| position | angleDirection | int | 角도方向，1또는-1 |
...
| position.excursion | Xexcursion | float | X方向偏이동 |
| position.excursion | Yexcursion | float | Y方向偏이동 |
| position.excursion | Zexcursion | float | Z方向偏이동 |
...
| robot | int | 로봇 번호 |
| visionNum | int | 프로세스번호 |
| type | int | 0: 테스트拍照点，1: 위치조정테스트点，2: 抓가져오기姿态基准点 |
...
| visionRange.maxX | string | X最大값을，未설정则이(가)空문자열"" |
| visionRange.maxY | string | Y最大값을 |
| visionRange.maxZ | string | Z最大값을 |
| visionRange.minX | string | X最小값을 |
| visionRange.minY | string | Y最小값을 |
| visionRange.minZ | string | Z最小값을 |
...
| 매개변수名 | 유형 | 설명 |
|--------|------|------|
| maxX | string | X最大값을，未설정则이(가)空문자열"" |
| maxY | string | Y最大값을 |
| maxZ | string | Z最大값을 |
| minX | string | X最小값을 |
| minY | string | Y最小값을 |
...
| - | calculation_type | int | 0: 眼에서手不변경姿态手眼캘리브레이션<br/>1: 眼에서手변경姿态手眼캘리브레이션<br/>2: 眼에서手이외手眼캘리브레이션 |
