...
| vision.protocol | addDataNum | int | 附加데이터个数 |
| vision.protocol | angleUnit | int | 0: 角度；1: 弧度 |
| vision.protocol | endMark | string | 结束符 |
| vision.protocol | failFlag | string | 失败전송标志符 |
...
| vision.protocol | successFlag | string | 成功전송标志符 |
...
| position | cameraPoint | array | 第一位X值，第二位Y值，第三位高度，第四位角度。<br/>若매개변수유형是2D和2D+H，则保存4元数组[x,y,H,angle]。<br/>若为3D，则保存6元数组[x,y,z,a,b,c]。 |
...
| position | recvPointsType | int | 接受위치유형：0: 点，1: 轨迹 |
...
| robot | int | 로봇 번호 |
| visionNum | int | 프로세스号，从0开始 |
| conveyorNum | int | 컨베이어프로세스号，从1开始 |
...
| robot | int | 로봇 번호 |
| visionNum | int | 프로세스号，从0开始 |
| originPos | array | 原始위치列表 |
| currentPos | array |当前위치列表 |
...
| - | robot | int | 로봇 번호 |
| - | visionNum | int | 프로세스号 |
| calibration | point_num | int | 캘리브레이션点个数 |
| calibration | calibrated | bool | 是否已캘리브레이션 |
...
| calibration | calibrated | bool | 是否已캘리브레이션 |
| calibration | point_num | int | 캘리브레이션点个数 |
| calibration.point | pixel_pos | array | 像素좌표弧度 |
| calibration.point | pixel_pos_deg | array | 像素좌표角度 |
| calibration.point | robot_pos | array | 로봇좌표弧度 |
| calibration.point | robot_pos_deg | array | 로봇좌표角度 |
| - | robot | int | 로봇 번호 |
| - | visionNum | int | 프로세스号 |
...
| pixel_pos_deg | array | 像素좌표角度制 |
| robot_pos_deg | array | 로봇좌표角度制 |
...
| visionRange.maxX | string | X最大值，未설정则为空字符串"" |
