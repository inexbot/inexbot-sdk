...
本目录包含로봇프로세스相关协议的完整文档，涵盖용접、스크류드라이빙、스프레이、그라인딩、팔레타이징、비전、심 추적、컨베이어跟踪、스탬핑、레이저 절단等多种프로세스유형。
...
| 프로세스유형 | 文档 | 설명 |
|---------|------|------|
| [[용접프로세스](./05.용접.md) | 0x4001~0x4015 | 용접装置、电流电压匹配、용접매개변수、摆焊매개변수、용접IO |
| [[스크류드라이빙프로세스](./03.스크류드라이빙.md) | 0x5731~0x5739 | 拧紧/拧松매개변수、IO상태、锁定结果 |
| [[스프레이프로세스](./07.스프레이.md) | 0x4701~0x4718 | 模拟量설정、数字量설정、轨迹매개변수、手动조작 |
| [[그라인딩프로세스](./02.그라인딩.md) | 0x4601~0x4603 | 그라인딩매개변수설정与조회 |
| [[팔레타이징프로세스](./01.팔레타이징.md) | 0x4201~0x421F | 抓手매개변수、托盘매개변수、위치매개변수、工件매개변수、重叠모드、平面모드 |
| [[비전프로세스](./04.비전.md) | 0x4101~0x4117 | 비전매개변수、위치매개변수、调试点位、캘리브레이션、拍照 |
| [[심 추적프로세스](./10.심-추적.md) | 0x4130~0x4171 | 激光器매개변수、激光器캘리브레이션、寻位유형、跟踪유형、激光跟踪매개변수 |
| [[컨베이어跟踪](./09.컨베이어-추적.md) | 0x4801~0x4823 | 컨베이어매개변수、매개변수识别、实时조회、좌표계캘리브레이션、传感器위치캘리브레이션 |
| [[新版스탬핑프로세스](./08.스탬핑-V2.md) | 0x8000~0x80XX | 프로그램실행、料况설정、제어모드、生产数量、스탬핑使能、상태조회 |
| [[레이저 절단프로세스](./06.레이저-절단/index.md) | 0x4401~0x4423 | IO포트설정、全局매개변수、模拟量匹配、切割매개변수、点射매개변수、手动조작 |
...
| 0x4004 | CURVOLMATCH_SET | 설정电流电压匹配 |
| 0x4005 | CURVOLMATCH_INQUIRE | 조회电流电压匹配 |
| 0x4006 | CURVOLMATCH_RESPOND | 반환电流电压匹配 |
...
| 0x400E | WELDFACTCURVOL_SET | 설정电流电压匹配实际值 |
...
| 消息ID | 名称 | 설명 |
|--------|------|------|
| 0x5731 | SCREWDRIVER_PARM_SET | 修改스크류드라이빙매개변수 |
...
| 0x5738 | SCREWDRIVER_TWISTRES_INQUIRE | 스크류드라이빙锁定结果조회 |
| 0x5739 | SCREWDRIVER_TWISTRES_RESPOND | 전송锁定结果 |
...
| 0x4704 | SPRAY_DIGIT_PARM_SET | 설정数字量매개변수 |
| 0x4705 | SPRAY_DIGIT_PARM_INQUIRE | 조회数字量매개변수 |
| 0x4706 | SPRAY_DIGIT_PARM_RESPOND | 반환数字量매개변수 |
| 0x4707 | SPRAY_SEQUENTIAL_SET | 설정时序 |
| 0x4708 | SPRAY_SEQUENTIAL_INQUIRE | 조회时序 |
| 0x4709 | SPRAY_SEQUENTIAL_RESPOND | 반환时序 |
| 0x470A | SPRAY_TRAJECTORY_SET | 설정轨迹매개변수 |
| 0x470B | SPRAY_TRAJECTORY_INQUIRE | 조회轨迹매개변수 |
| 0x470C | SPRAY_TRAJECTORY_RESPOND | 반환轨迹매개변수 |
...
| 0x420A | PAL_WORKPIECE_PARM_SET | 설정工件매개변수 |
| 0x420B | PAL_WORKPIECE_PARM_INQUIRE | 조회工件매개변수 |
| 0x420C | PAL_WORKPIECE_PARM_RESPOND | 반환工件매개변수 |
...
| 0x4107 | VISION_DEBUGGING_POS_INQUIRE | 조회비전调试点位列表 |
| 0x4108 | VISION_DEBUGGING_POS_RESPOND | 반환调试点位列表 |
...
| 0x4116 | VISION_GESTURE_CALIBRATION_CLEAR | 清除抓取姿态캘리브레이션 |
...
| 消息ID | 名称 | 설명 |
|--------|------|------|
| 0x4130 | TRACK_LASER_PARAM_SET | 설정激光器매개변수 |
| 0x4131 | TRACK_LASER_PARAM_INQUIRE | 조회激光器매개변수 |
| 0x4132 | TRACK_LASER_PARAM_RESPOND | 반환激光器매개변수 |
| 0x4133 | LOCATING_SENSORTYPE_SET | 설정寻位유형 |
| 0x4134 | LOCATING_SENSORTYPE_INQUIRE | 조회寻位유형 |
| 0x4135 | LOCATING_SENSORTYPE_RESPOND | 반환寻位유형 |
| 0x4136 | TRACK_LASER_TRACKPARAM_SET | 설정激光跟踪매개변수表 |
...
| 0x4149 | SENSOR_LASER_CALIBRATE_RESULT_INQUIRE | 조회激光器是否캘리브레이션 |
| 0x414A | SENSOR_LASER_CALIBRATE_RESULT_RESPOND | 반환캘리브레이션结果 |
| 0x4169 | TRACK_SENSORTYPE_SET | 설정跟踪유형 |
| 0x4170 | TRACK_SENSORTYPE_INQUIRE | 조회跟踪유형 |
| 0x4171 | TRACK_SENSORTYPE_RESPOND | 반환跟踪유형 |
...
| 0x4805 | TRACK_CONVEYOR_POSCHECKPARAM_INQUIRE | 조회工件识别매개변수 |
| 0x4806 | TRACK_CONVEYOR_POSCHECKPARAM_RESPOND | 반환工件识别매개변수 |
| 0x4807 | TRACK_CONVEYOR_REALTIME_INQUIRE | 实时조회编码器值和속도 |
...
| 0x8006 | PUNCH_INQUIRE_PRODUCE_TARGET_WASTE_NUM | 生产数量조회 |
| 0x8007 | PUNCH_RESPOND_PRODUCE_TARGET_WASTE_NUM | 반환生产数量 |
| 0x8008 | PUNCH_CLEAR_PRODUCE_TARGET_WASTE_NUM | 生产数量清零 |
| 0x8009 | PUNCH_SET_SHIELD_CHECK_RUN_WITHOUT_PIECE_PUNCH_ENABLE | 스탬핑使能설정 |
| 0x800A | PUNCH_INQUIRE_SHIELD_CHECK_RUN_WITHOUT_PIECE_PUNCH_ENABLE | 조회스탬핑使能 |
| 0x800B | PUNCH_RESPOND_SHIELD_CHECK_RUN_WITHOUT_PIECE_PUNCH_ENABLE | 반환스탬핑使能 |
...
| 0x8011 | PUNCH_ONLINE_STATUS_RESET | 联机상태复位 |
| 0x8012 | PUNCH_JOBFILE_OPERATION | 暂停/启动/停止 |
| 0x8013 | PUNCH_INQUIRE_RUN_STATUS | 실행周期조회 |
| 0x8014 | PUNCH_RESPOND_RUN_STATUS | 반환실행상태 |
| 0x8015 | PUNCH_SEND_ERROR_INFO | 전송错误弹窗信息 |
| 0x8016 | PUNCH_RECEIVE_ERROR_DEAL | 수신弹窗按键处理 |
| 0x8017 | PUNCH_CHOOSE_CRAFT_NUM | 选中프로세스号 |
| 0x8018 | PUNCH_APPLY_TO_ONLINE | 应用到联机 |
| 0x8019 | PUNCH_INQUIRE_CURRENT_CRAFT_NUM | 조회当前选中프로세스号 |
| 0x801A | PUNCH_RESPOND_CURRENT_CRAFT_NUM | 반환当前选中프로세스号 |
...
| 0x4407 | LASER_ANALOGMATCH_SET | 模拟量匹配설정 |
| 0x4408 | LASER_ANALOGMATCH_INQUIRE | 模拟量匹配조회 |
| 0x4409 | LASER_ANALOGMATCH_RESPOND | 模拟量匹配응답 |
| 0x440A | LASER_CUTPARM_SET | 切割매개변수설정 |
| 0x440B | LASER_CUTPARM_INQUIRE | 切割매개변수조회 |
| 0x440C | LASER_CUTPARM_RESPOND | 切割매개변수응답 |
...
2. **必填매개변수**：매개변수 설명中的"必填"列表示该매개변수是否必须提供
